...
func (r1 BotaReading) Sub(r2 BotaReading) BotaReading {
    return BotaReading{
        Fx:          r1.Fx - r2.Fx,
        Fy:          r1.Fy - r2.Fy,
        Fz:          r1.Fz - r2.Fz,
...
type BotaFilter struct {
    ...
    window int
    postWindow int
    median *signal.MedianFilter[float32]
    postMedian *signal.MedianFilter[float32]

    // kalman filter
    pVar float64
    mVar float64
    eCovar float64
    kalman *signal.KalmanFilter[float32]
    
    threshold float64
    fzInit float32
}

func NewBotaFilter(window, postWindow int, threshold float64, pVar, mVar, eCovar float64, readingInit float32) *BotaFilter {
    return &BotaFilter{
        window: window,
        postWindow: postWindow,
        median: signal.NewMedianFilter[float32](window),
        postMedian: signal.NewMedianFilter[float32](postWindow),
        pVar: pVar,
        mVar: mVar,
        eCovar: eCovar,
        kalman: signal.NewKalmanFilter(pVar, mVar, eCovar, readingInit),
        threshold: threshold,
        fzInit: readingInit,
    }
}

func (f *BotaFilter) Compute(reading BotaReading) float32 {
    fz := f.median.Compute(reading.Fz)
    fz = f.kalman.Compute(fz)
    fz -= f.fzInit

    if math.Abs(float64(fz)) < f.threshold {
        fz = 0
        f.postMedian = signal.NewMedianFilter[float32](f.postWindow)
        return fz
    }

    fz = f.postMedian.Compute(fz)

    return fz
}

func (f *BotaFilter) Reset(readingInit float32) {
    f.median = signal.NewMedianFilter[float32](f.window)
    f.postMedian = signal.NewMedianFilter[float32](f.postWindow)
    f.kalman = signal.NewKalmanFilter(f.pVar, f.mVar, f.eCovar, readingInit)
    f.fzInit = readingInit
}
...
func (b *BotaSensor) Read() BotaReading {
    b.mux.RLock()
    defer b.mux.RUnlock()
    return b.reading
}
